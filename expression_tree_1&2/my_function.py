    ...
    for i in range(len(text_list)):
        ...
        if len(var_list[i]) != 0: 
            ...
            for j in range(2**len(var_list[i])):
                ...
                sub_expression_list.append(temp)
                temp = text_list[i]
                c += 1
        else:
            sub_expression_list.append(text_list[i])
            expression_list.append(sub_expression_list)
            sub_expression_list = []
        if sub_expression_list != []:
            expression_list.append(sub_expression_list)
...
def output_list(text_list):
    var_list = count_var(text_list)
    ep = expression_list(text_list,var_list)
    output = []
    for i in range(len(ep)):
        temp = []
        for j in range(len(ep[i])):
            temp.append(calculate_expression(ep[i][j]))
        output.append(temp)
    return output
    
def print_output(text_list):
    var = count_var(text_list)
    out = calculate_expression(text_list)
